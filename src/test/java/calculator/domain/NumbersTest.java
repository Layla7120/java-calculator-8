...
class NumbersTest {
    ...
    void create_numbers_success() {
        assertThatCode(() -> new Numbers(new String[]{"1", "2", "3"})).doesNotThrowAnyException();
    }

    @Test
    @DisplayName("음수가 포함된 경우 예외 발생")
    void create_numbers_with_negative() {
        assertThatThrownBy(() -> new Numbers(new String[]{"1", "-2", "3"})).isInstanceOf(IllegalArgumentException.class)
    ...
    void create_numbers_with_non_numeric() {
        assertThatThrownBy(() -> new Numbers(new String[]{"1", "a", "3"})).isInstanceOf(IllegalArgumentException.class)
    ...
    void create_numbers_with_blank() {
        assertThatThrownBy(() -> new Numbers(new String[]{"1", "", "2"})).isInstanceOf(IllegalArgumentException.class)
    ...
    void create_numbers_with_negative_and_custom_delimiter() {
        assertThatThrownBy(() -> new Numbers(new String[]{"1", "-5", "3"})).isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("음수 값이 포함되어 있습니다");
    }
}
